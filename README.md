...
The A2D adapter positively detects live PICO/VR activity through a bounded, read-only
SSH process check. It verifies the streamer, UDP listener, and a short CPU-tick delta
without connecting to rosbridge. Positive activity marks VR active; every inconclusive
result remains unknown and fail-closed. OpenRoboOps never disables collision protection
and does not expose raw WBC/MBC publishing endpoints.
