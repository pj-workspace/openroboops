...
A2D adapter 通过有界、只读的 SSH 进程检查正向探测 PICO/VR 活跃状态，检查
streamer、UDP 监听和短时间 CPU tick 变化，不连接 rosbridge。明确探测到活动时
标记为活跃；所有无法确认的结果仍为未知并 fail closed。OpenRoboOps 永远不会
关闭碰撞保护，也不暴露原始 WBC/MBC 发布接口。
